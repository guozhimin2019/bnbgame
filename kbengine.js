...
KBEngine.Entity = KBEngine.Class.extend(
{
	...
	baseCall : function()
	{
		...
		var args = method[3];
		console.log(`send cmd:[${arguments[0]}]`,method);
		// console.log("send cmd:" +arguments[0]+" "+method[0]+" "+method[1]+" "+method[2]+" "+method[3]);
...
KBEngine.KBEngineApp = function(kbengineArgs)
{
	...
	this.serverErrs = {};
	this.pingTime = 0;//心跳时间
	this.connectCount = 0;//重连次数
	this.diconnectState = true;
	// 登录loginapp的地址
	...
	this.reset = function()
	{
		...
		KBEngine.app.clientVersion = "2.5.8";
	...
	{  
		...
		KBEngine.app.connectCount = 0;
		KBEngine.app.diconnectState = false;
		__config.bLogined = true;
		__model.ui.closeUI(__model.ui.page.ui_reconnect);
	...
	this.reconnect = function(){
		KBEngine.app.connectCount++;
		KBEngine.app.resetSocket();
		KBEngine.app.diconnectState = true;
		__game.info.diconnectState = true;
		if(!__config.bLogined){//手动退出登录不重连
			KBEngine.app.connectCount = 0;
		}
		else if (KBEngine.app.connectCount > 10) {
			KBEngine.app.connectCount = 0;
			__model.ui.closeUI(__model.ui.page.ui_reconnect);
			KBEngine.Event.fire(KBEngine.EventTypes.onConnectionState, false);
			...
			//断线重连
			__model.ui.show(__model.ui.page.ui_reconnect);
			setTimeout(() => {
				if (!KBEngine.app.socket) {
					let localName = cc.sys.localStorage.getItem("UserName");
					let localPw = cc.sys.localStorage.getItem("UserPw");
					KBEngine.Event.fire("login", localName, localPw, "kbengine_unity3d_balls");
				}
			}, 2000);
		}
	}
	...
	{
		if(!KBEngine.app.socket){
			return console.error('send is null');
		}
	...
	this.update = function()
	{
		...
		var dateObject = new Date();
		if(dateObject.getTime() > KBEngine.app.pingTime){
			KBEngine.app.pingTime = dateObject.getTime() + 25000;
			let avatar = KBEngine.app.player();	
			if(avatar){
				avatar.baseCall(__post.ping);
			}
		}

		return;

		if(KBEngine.app.args.serverHeartbeatTick > 0 && (dateObject.getTime() - KBEngine.app.lastTickTime) / 1000 > (KBEngine.app.args.serverHeartbeatTick))
		{
			// console.log("beat nowtime: "+ dateObject.getTime() +" lastTickTime: "+ KBEngine.app.lastTickTime+" span: "+ (dateObject.getTime() - KBEngine.app.lastTickTime)+" "+(KBEngine.app.args.serverHeartbeatTick));
	...
	*/
	this.Client_onAppActiveTickCB = function()
	{
		...
		// console.log("服务器回调时间："+KBEngine.app.lastTickCBTime);
	}
	...
	*/
	...
	this.Client_onImportClientEntityDef = function(stream)
	{
		...
		while(stream.length() > 0)
		{
			...
			var cls = KBEngine[scriptmodule_name];
			while(propertysize > 0)
			{
				...
				if(cls != undefined)
				{
					setmethod = cls.prototype["set_" + name];
	...
	this.getServerAddr = function(ip, port)
	{
		...
		// var serverAddr = KBEngine.app.protocol + 'web.bnbgame.xyz'; 
		// console.log("getserveraddr: "+ip);
		// var serverAddr = null;
		// if(cc.PlatForm == cc.PlatformEnum.H5){
		// 	serverAddr = KBEngine.app.protocol+ip;
		// }else if(cc.PlatForm ==cc.PlatformEnum.Android){
		// 	serverAddr = ip;
		// }else if(cc.PlatForm == cc.PlatformEnum.IOS){
		// 	serverAddr = ip;
		// }
	...
	{  
		if(noconnect)
		{
			...
			// var serverAddr = null;
			// if(cc.PlatForm == cc.PlatformEnum.H5){
			// 	serverAddr = this.getServerAddr("web.bnbgame.xyz",KBEngine.app.baseappPort);
			// }else if(cc.PlatForm ==cc.PlatformEnum.Android){
			// 	serverAddr = this.getServerAddr(KBEngine.app.baseappIp, KBEngine.app.baseappPort);
			// }else if(cc.PlatForm == cc.PlatformEnum.IOS){
			// 	serverAddr = this.getServerAddr(KBEngine.app.baseappIp, KBEngine.app.baseappPort);
			// }
	...
	this.Client_onCreatedProxies = function(rndUUID, eid, entityType)
	{
		...
		KBEngine.app.entity_type = entityType;
